...
**/
    ...
    tBTNodePtr tmp, tmproot = *RootPtr;
    bool found = false;
    if (*RootPtr == NULL) {
        tmp = NULL;
        found = false;
        ...
        do {
            tmp = tmproot;
            if (tmproot->Cont > Content) {
                tmproot = tmproot->LPtr;
            }
            else if (tmproot->Cont < Content) {
                tmproot = tmproot->RPtr;
            }
            else {
                found = true;
            }
        } while ((found == false) && (tmproot != NULL));
    }
...
**/
    ...
    tStackP stack_p;
    SInitP(&stack_p);
    Leftmost_Preorder(RootPtr, &stack_p);
    while (SEmptyP(&stack_p) == false) {
        RootPtr = STopPopP(&stack_p);
        Leftmost_Preorder(RootPtr->RPtr, &stack_p);
    }
}
...
**/
    ...
    while (SEmptyP(&stack_p) == false) {
        ...
        if (RootPtr->RPtr != NULL) {
            Leftmost_Inorder(RootPtr->RPtr, &stack_p);
        }
    }
}
...
**/
    ...
    if (*RootPtr != NULL) {
        ...
        do {
            if (*RootPtr == NULL) {
                if (SEmptyP(&stack_p) == false) {
                    *RootPtr = STopPopP(&stack_p);
                }
            }
            else {
                if ((*RootPtr)->RPtr != NULL) {
                SPushP(&stack_p, (*RootPtr)->RPtr);
                }
                tBTNodePtr tmp;
                tmp = *RootPtr;
        ...
        } while (((*RootPtr) != NULL) || (SEmptyP(&stack_p) == false));
    }
}
