...
*/
    return (s->top < 0); //Pokud je prazdny vrati false(0), pokud neni vrati true(1)
}
...
*/
    ...
    return (s->top+1 == STACK_SIZE); //Pokud je plny vraci true(1), jinak false(0)
}
...
*/
    if (stackEmpty(s)) {
        stackError(SERR_TOP); //Pokud je prazdny, chyba
    }
    else {
        *c = s->arr[s->top]; //Vrati hodnotu odpovidajiciho prvku (index v poli)
    }
}
...
*/
    if (!stackEmpty(s)) {
        s->top--; //Pokud neni prazdny smaze prvek
    }
}
